  ...
  useEffect(() => {
    const fetchRecipes = async () => {
      try {
        ...
        const mappedRecipes: Recipe[] = data.data.map((item: any) => ({
          id: item.id,
    ...
    const filteredRecipes = recipes.filter((recipe) => {
      ...
      const matchesTags =
        selectedTags.length === 0 || selectedTags.every((tag) => recipe.tags.some(recipeTag => recipeTag.toLowerCase() === tag.toLowerCase()));
    ...
    return (
      ...
      <motion.div layout className="grid grid-cols-2 gap-6">
        {filteredRecipes.map((recipe) => (
          ...
          <RecipeCard key={recipe.id} recipe={{ ...recipe, url: `/recipes/${recipe.documentId}` }} />
          </motion.div>
        ))}
      </motion.div>
      </AnimatePresence>
